        ...
        with request.urlopen(url, timeout=6) as response:
            ...
            for mirror in json.loads(content):
                yield (
                    ...
                    mirror["country"],
                    "9999:00"
                    if isinstance(mirror["last_sync"], int)
                    else mirror["last_sync"],
    ...
    def data(self, index, role: Qt.ItemDataRole):
        ...
        if role == Qt.ItemDataRole.ToolTipRole and col == 2:
            return (
                ...
                if self._data[index.row()][2] != "9999:00"
                else "none"
            )
...
class MirrorsWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None):
        ...
        label = QLabel(
            f"Local mirror status for <b>{self.branch.name}</b> branch",
            parent=self,
            margin=10,
        )
